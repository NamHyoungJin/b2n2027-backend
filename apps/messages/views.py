    ...
    def post(self, request):
        ...
        if is_scheduled_request:
            if scheduled_at is None:
                return Response(
                    {"detail": "예약 발송은 예약 일시(scheduled_at)가 필요합니다."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            min_allowed = now + timedelta(minutes=10)
            if scheduled_at < min_allowed:
                return Response(
                    {"detail": "예약 발송 시간은 현재 기준 최소 10분 이후여야 합니다."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        ...
        with transaction.atomic():
            batch = MessageBatch.objects.create(
                **payload,
                ...
                requested_at=now,
                created_by_id=_admin_sid(request),
            )
            details = []
...
class MessageBatchDetailView(APIView):
    ...
    def get(self, request, batch_id: int):
        try:
            ...
            return Response({"detail": "배치를 찾을 수 없습니다."}, status=status.HTTP_404_NOT_FOUND)
...
class MessageBatchCancelView(APIView):
    ...
    def post(self, request, batch_id: int):
        ...
        if batch.status != MessageBatch.STATUS_SCHEDULED:
            return Response({"detail": "예약 상태에서만 취소 가능합니다."}, status=status.HTTP_400_BAD_REQUEST)
...
class MessageBatchResendFailedView(APIView):
    ...
    def post(self, request, batch_id: int):
        try:
            ...
            return Response({"detail": "원본 배치를 찾을 수 없습니다."}, status=status.HTTP_404_NOT_FOUND)

        failed_details = list(source.details.filter(status=MessageDetail.STATUS_FAIL))
        if not failed_details:
            return Response({"detail": "재전송할 실패 건이 없습니다."}, status=status.HTTP_400_BAD_REQUEST)
...
class MessageSenderNumberListCreateView(APIView):
    ...
    def post(self, request):
        ...
        if existing and existing.deleted_at is not None:
            ...
            existing.created_by_id = _admin_sid(request)
            existing.save()
            out = MessageSenderNumberSerializer(existing).data
            return Response(out)

        if existing and existing.deleted_at is None:
            return Response({"detail": "이미 등록된 발신번호입니다."}, status=status.HTTP_400_BAD_REQUEST)

        obj = serializer.save(
            created_by_id=_admin_sid(request),
            processed_at=timezone.now()
            if serializer.validated_data.get("status") == MessageSenderNumber.STATUS_APPROVED
            else None,
        )
...
class MessageSenderNumberDeleteView(APIView):
    ...
    def delete(self, request, sender_id: int):
        try:
            ...
            return Response({"detail": "발신번호를 찾을 수 없습니다."}, status=status.HTTP_404_NOT_FOUND)
...
class MessageSenderEmailListCreateView(APIView):
    ...
    def post(self, request):
        ...
        if existing and existing.deleted_at is None:
            return Response({"detail": "이미 등록된 발신 이메일입니다."}, status=status.HTTP_400_BAD_REQUEST)
...
class MessageSenderEmailDeleteView(APIView):
    ...
    def delete(self, request, sender_id: int):
        try:
            ...
            return Response({"detail": "발신 이메일을 찾을 수 없습니다."}, status=status.HTTP_404_NOT_FOUND)
        obj.deleted_at = timezone.now()
        obj.save(update_fields=["deleted_at", "updated_at"])
        return Response(status=status.HTTP_204_NO_CONTENT)
...
class MessageTemplateListCreateView(APIView):
    ...
    def post(self, request):
        ...
        if not serializer.is_valid():
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        obj = serializer.save(created_by_id=_admin_sid(request))
...
class AligoRemainView(APIView):
    ...
    def get(self, request):
        ...
        if not api_key or not user_id:
            return Response(
                {"detail": "ALIGO_API_KEY 또는 ALIGO_USER_ID가 설정되지 않았습니다."},
        ...
        try:
            ...
            return Response(
                {"detail": "알리고 잔여건수 조회 중 네트워크 오류가 발생했습니다."},
                status=status.HTTP_502_BAD_GATEWAY,
            )
        except ValueError:
            return Response(
                {"detail": "알리고 응답을 해석할 수 없습니다."},
                status=status.HTTP_502_BAD_GATEWAY,
            )
        ...
        if code != "1":
            return Response(
                {"detail": payload.get("message") or "알리고 잔여건수 조회 실패"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class MessageBatchSyncResultView(APIView):
    ...
    def post(self, request, batch_id: int):
        ...
        if batch.type != MessageBatch.TYPE_SMS:
            return Response({"detail": "문자(sms) 배치만 결과 동기화 가능합니다."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if not msg_id:
            return Response({"detail": "알리고 msg_id가 없어 동기화할 수 없습니다."}, status=status.HTTP_400_BAD_REQUEST)

        sync_result = fetch_sms_list_all(msg_id)
        if not sync_result.get("ok"):
            return Response(
                {"detail": str(sync_result.get("message") or "동기화 실패")},
                status=status.HTTP_502_BAD_GATEWAY,
            )

        rows = sync_result.get("list") or []
        details = list(batch.details.all())
        detail_by_phone: dict[str, list] = {}
        for d in details:
        ...
        for row in rows:
            ...
            for d in detail_by_phone[phone]:
                ...
                d.save(
                    update_fields=["status", "external_message", "external_code", "sent_at", "error_reason", "updated_at"]
                )
                updated += 1
